    ...
    function run_timestep(p, v, d, t)
        if !is_first(t)
            for r in d.regions
                ...
                v.tempstressdeadcost[t, r] = p.population[t,r]*1000000*(p.alphamortality[r]+(p.betaonemortality[r] * (p.regtmp[t,r])+ p.betatwomortality[r]*(p.regtmp[t,r])^2)*((ypc)^(p.cmortality[r])))*(ypc/100)
